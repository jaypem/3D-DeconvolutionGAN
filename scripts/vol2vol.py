...
from skimage import io # später löschen
...
class Pix3Pix():
    ...
    def save_table_images(self, epoch, batch_i, p):
        ...
        for i in range(r):
            ...
            for j in range(c):
                ...
                ### extract entire images for thesis
                import cv2
                temp_img = np.max(gen_imgs[j,:,:,:], axis=i)
                temp_img = cv2.resize(temp_img, (256, 256)) 
                io.imsave('{0}/{1}_{2}_{3}_{4}_IMG.png'.format(directory, epoch, batch_i, dims[i], titles[j]), temp_img)
                #cv2.imwrite('{0}/{1}_{2}_{3}_{4}_IMG.png'.format(directory, epoch, batch_i, dims[i], titles[j]), np.max(gen_imgs[j,:,:,:], axis=i))
                ### extract entire images for thesis
    ...
    def save_volume(self, epoch, batch_i, p):
        ...
        tiff.imsave('{0}/{1}_{2}_Original.tif'.format(directory, epoch, batch_i), vol_A.astype(np.float32))
        tiff.imsave('{0}/{1}_{2}_OTF.tif'.format(directory, epoch, batch_i), vol_B.astype(np.float32))
        tiff.imsave('{0}/{1}_{2}_Generated.tif'.format(directory, epoch, batch_i), fake_A.astype(np.float32))
